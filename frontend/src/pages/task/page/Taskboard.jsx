...
function Taskboard() {
  ...
  const renderTaskCard = (task) => {
    ...
    return (
      <div
        key={task.id}
        className={`flex flex-col w-full h-32 p-2 ${getStatusColor(task.status)} border rounded-lg shadow text-left justify-center ${isUpdateMode ? 'cursor-grab active:cursor-grabbing' : ''} ${draggedTask?.id === task.id ? 'opacity-50' : ''} ${getStatusMargin(task.status)}`}
